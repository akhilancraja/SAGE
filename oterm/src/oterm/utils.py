...
def list_local_gguf_models(models_path: Path) -> list[str]: # Custom Model Loading Utility
    """
    Discover .gguf model files in the given directory and return their names (without extension).
    """
    if not models_path.exists():
        return []
    return [f.stem for f in models_path.glob("*.gguf")]
